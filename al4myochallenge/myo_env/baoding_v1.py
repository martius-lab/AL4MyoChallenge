...
class BaodingEnvV1(BaseV0):
    ...
    def _setup(
        ...
        goal_yrange=(
            0.028,
            0.028,
        ),  # target rotation: x radius (0.02 * 1.5 * 1.2)
        ...
        **kwargs,
        ...
        self.which_task = (
            self.np_random.choice(Task)
            if task_choice == "random"
            else Task(WHICH_TASK)
        )
        self.drop_th = drop_th
        self.proximity_th = proximity_th
        self.proximity_th_tight = 0.0075
        ...
        self.obj_mass_range = {
            "low": obj_mass_range[0],
            "high": obj_mass_range[1],
        }
        self.obj_size_range = {
            "low": obj_size_range[0],
            "high": obj_size_range[1],
        }
    ...
    def step(self, a):
        if self.which_task in [Task.HOLD, Task.BAODING_CW, Task.BAODING_CCW]:
            ...
            desired_positions_wrt_palm[0] = (
                self.x_radius * np.cos(desired_angle_wrt_palm[0])
                + self.center_pos[0]
            )
            desired_positions_wrt_palm[1] = (
                self.y_radius * np.sin(desired_angle_wrt_palm[0])
                + self.center_pos[1]
            )
            desired_positions_wrt_palm[2] = (
                self.x_radius * np.cos(desired_angle_wrt_palm[1])
                + self.center_pos[0]
            )
            desired_positions_wrt_palm[3] = (
                self.y_radius * np.sin(desired_angle_wrt_palm[1])
                + self.center_pos[1]
            )
        ...
        self.counter += 1
        ret = super().step(a)
        self.old_qpos = self.obs_dict["hand_pos"].copy()
        self.old_acts = self.obs_dict["act"].copy()
        return ret
    ...
    def get_obs_dict(self, sim):
        ...
        obs_dict["target1_err"] = (
            obs_dict["target1_pos"] - obs_dict["object1_pos"]
        )
        obs_dict["target2_err"] = (
            obs_dict["target2_pos"] - obs_dict["object2_pos"]
        )

        # my qvel -----------------------------------------------
        if not hasattr(self, "old_qpos"):
            self.old_qpos = np.zeros_like(obs_dict["hand_pos"])
        if np.all(self.old_qpos == 0):
            obs_dict["hand_vel"] = self.old_qpos
        else:
            obs_dict["hand_vel"] = obs_dict["hand_pos"] - self.old_qpos
    ...
    def get_reward_dict(self, obs_dict):
        ...
        acts = self.obs_dict["act"]
        act_mag = (
            np.linalg.norm(acts, axis=-1) / self.sim.model.na
            if self.sim.model.na != 0
            else 0
        )
        act_mag_square = (
            np.mean(np.power(acts, 2), axis=-1) / self.sim.model.na
            if self.sim.model.na != 0
            else 0
        )
        act_mag_series = np.mean(
            np.sum([np.power(acts, k) for k in range(1, 5)])
        )
        ...
        rwd_dict = collections.OrderedDict(
            (
                ...
                # Must keys
                ("act_reg", -1.0 * act_mag),
                ("act_square", -1.0 * act_mag_square),
                ("act_series", -1.0 * act_mag_series),
                ("sparse", -target_dist),
                (
                    "solved",
                ...
                (
                    "solved_tight",
                    (target1_dist < self.proximity_th_tight)
                    * (target2_dist < self.proximity_th_tight)
                    * (~is_fall),
                ),
                ("done", is_fall),
            )
    ...
    def evaluate_success(self, paths, logger=None, successful_steps=5):
        ...
        score = np.mean(
            [
                np.sum(p["env_infos"]["rwd_dict"]["solved"]) / self.horizon
                for p in paths
            ]
        )
    ...
    def reset(
        ...
        self.goal = (
            self.create_goal_trajectory(
                time_step=self.dt, time_period=time_period
            )
        ...
        # call to mj_setConst(m,d) is being ignored. Derive quantities wont be updated.
        # Die is simple shape. So this is reasonable approximation.
    ...
    def create_goal_trajectory(self, time_step=0.1, time_period=6):
        ...
        while t < len_of_goals:
            angle_before_shift = (
                sign * 2 * np.pi * (t * time_step / time_period)
            )
            goal_traj.append(
